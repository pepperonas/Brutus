 ...
 * compliant and it's default configuration is also compatible with <a
 * href="https://play.google.com/store/apps/details?id=com.google.android.apps.authenticator2&hl=en">Google
 * Authenticator</a>.
 *
 * <p>This allows for easy 2 step authentication integration with own service without necessity to
 * write own code generating application
 *
...
public class TotpManager {
    ...
    final String generateOTP(byte[] secret, long itvl) {
        ...
        int bin =
            ((hash[off] & 0x7f) << 24) | ((hash[off + 1] & 0xff) << 16) | ((hash[off + 2] & 0xff)
                << 8) | (hash[off + 3] & 0xff);
